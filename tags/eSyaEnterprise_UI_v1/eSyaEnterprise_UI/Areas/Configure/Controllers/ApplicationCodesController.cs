...
namespace eSyaEnterprise_UI.Areas.Configure.Controllers
{
    ...
    public class ApplicationCodesController : Controller
    {
        ...
        public async Task<JsonResult> GetCodeTypes()
        {
            ...
            try
            {
                var serviceResponse = await _configAPIServices.HttpClientServices.GetAsync<List<DO_CodeTypes>>("ApplicationCodes/GetCodeTypes");
        ...
        public async Task<JsonResult> InsertOrUpdateCodeTypes(bool isInsert, DO_CodeTypes ct_type)
        {
            ...
            try
            {
                ...
                if (isInsert)
                {
                    var serviceResponse = await _configAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("ApplicationCodes/InsertIntoCodeType", ct_type);
                    if (serviceResponse.Status)
                        return Json(serviceResponse.Data);
                    else
                ...
                {
                    var serviceResponse = await _configAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("ApplicationCodes/UpdateCodeType", ct_type);
                    if (serviceResponse.Status)
                        return Json(serviceResponse.Data);
                    else
                        return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
        ...
        public async Task<IActionResult> ECA_04_00()
        {
            try
            {
                var serviceResponse = await _configAPIServices.HttpClientServices.GetAsync<List<DO_CodeTypes>>("ApplicationCodes/GetActiveCodeTypes");
                if (serviceResponse.Status)
                {
        ...
        public async Task<JsonResult> ActiveOrDeActiveApplicationCode(bool status, int app_code)
        {
            ...
            try
            {
                ...
                var serviceResponse = await _configAPIServices.HttpClientServices.GetAsync<DO_ReturnParameter>("ApplicationCodes/ActiveOrDeActiveApplicationCode" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
        ...
        public async Task<IActionResult> ESA_01_00()
        {
            try
            {
                var serviceResponse = await _configAPIServices.HttpClientServices.GetAsync<List<DO_CodeTypes>>("ApplicationCodes/GetUserDefinedCodeTypesList");
                if (serviceResponse.Status)
                {
                    ViewBag.CodeTypeList = serviceResponse.Data.Select(b => new SelectListItem
                    {
