...
namespace eSyaEnterprise_UI.Areas.eSyaLaboratory.Controllers
{
    ...
    public class ServiceTemplateCreatorController : Controller
    {
        ...
        public async Task<IActionResult> V_5000_00()
        {
        ...
        public async Task<ActionResult> GetFullServiceResultTemplate(int businessKey, int serviceID, string resultType,string templateType)
        {
            try
            {
                var serviceResponse = await _eSyaLaboratoryAPIServices.HttpClientServices.GetAsync<DO_ServiceTemplateFull>("ServiceTemplateCreator/GetFullServiceResultTemplate?businessKey=" + businessKey + "&serviceID=" + serviceID +"&resultType=" + resultType + "&templateType=" + templateType);
                if (serviceResponse.Status)
                {
                    if (serviceResponse.Data != null)
                    {
