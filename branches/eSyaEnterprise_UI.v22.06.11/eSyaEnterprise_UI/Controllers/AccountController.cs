...
namespace eSyaEnterprise_UI.Controllers
{
    ...
    public class AccountController : Controller
    {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (serviceResponse.Status)
                    {
                        ...
                        else
                        {
                            ...
                            AppSessionVariables.SetSessionBusinessKey(HttpContext, Convert.ToInt32(l_b.FirstOrDefault().Value));
            ...
            try
            {
                ...
                if (serviceResponse.Status)
                {
                    if (serviceResponse.Data.IsSucceeded)
                    {
                        ...
                        sms.TEventID = SMSTriggerEventValues.MobileLoginOTP;
        ...
        public async Task<JsonResult> CreateUserPassword(int userID, string newPassword, string confirmPassword)
        {
            ...
            try
            {
                var obj = new { UserID = userID, Password = newPassword };
                var serviceResponse = await _eSyaGatewayServices.HttpClientServices.PostAsJsonAsync<DO_UserAccount>("UserAccount/CreateUserPassword", obj);
                if (serviceResponse.Status)
                {
