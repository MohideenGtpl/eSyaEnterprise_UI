...
function fnGridLoadToken() {
    ...
    $("#jqgToken").jqGrid({
        ...
        colNames: [localization.TokenType, localization.TokenDescription, localization.TokenPrefix, localization.TokenNumber, localization.Active, localization.Actions],
        colModel: [
            { name: "TokenType", width: 50, align: 'left', editable: true, editoptions: { maxlength: 15 }, resizable: false, hidden: false },
            { name: "TokenDesc", width: 120, align: 'left', editable: true, editoptions: { maxlength: 50 }, resizable: false, hidden: false },
...
function fnSaveToken() {
    ...
    if (IsStringNullorEmpty($("#txtTokenType").val())) {
        toastr.warning("Please Enter Token Type");
        return;
    }
    if (IsStringNullorEmpty($("#txtTokenDescription").val())) {
        toastr.warning("Please Enter Token Description");
        return;
    }
    if (IsStringNullorEmpty($("#txtTokenPrefix").val())) {
        toastr.warning("Please Enter Token Prefix");
        return;
    }
    if (IsStringNullorEmpty($("#txtTokenNumber").val())) {
        toastr.warning("Please Enter Token Number");
        return;
    }
    objtoken = {
