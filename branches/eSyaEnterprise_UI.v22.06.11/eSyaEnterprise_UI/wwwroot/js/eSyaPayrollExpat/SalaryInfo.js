...
function fnAddToGrid() {
    ...
    if ($("#txtAmount").val().trim().length <= 0) {
        toastr.warning("Please Enter txtAmount");
        $('#txtAmount').focus();
        return;
    }
    if ($("#cboTransferTo").val() === "0" || $("#cboTransferTo").val() === "") {
        toastr.warning("Please Select Trasnfer To");
        $('#cboTransferTo').focus();
        return;
    }
