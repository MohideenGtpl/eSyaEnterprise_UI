...
namespace eSyaEnterprise_UI.Areas.eSyaPayrollExpat.Controllers
{
    ...
    public class VariableEntryController : Controller
    {
        ...
        [HttpPost]
        public async Task<JsonResult> InsertOrUpdateVariableIncentive(List<DO_VariableIncentive> obj)
        {
            try
            {
                obj.All(c =>
                {
                    ...
                    var serviceResponse = await _eSyaPayrollExpatAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("VariableEntry/InsertOrUpdateVariableIncentive", obj);
                    if (serviceResponse.Status)
                        return Json(serviceResponse.Data);
                    else
                        return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
