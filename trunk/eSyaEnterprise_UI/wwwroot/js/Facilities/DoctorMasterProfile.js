...
function fnEditDoctorProfile(e) {
    ...
    $("#btnSaveDoctorProfile,#btnSaveAboutDoctor,#btnSaveDoctorProfileImage, #btnSaveDoctorProfileBusinessLink, #btnSaveDoctorProfileAddress, #btnSaveDoctorStatutoryDetails,#btnSaveDoctorConsultationRate, #btnClearDoctor").html("<i class='fa fa-sync'></i> Update");
    $("#divGrid").hide();
    $('#divDoctorProfileForm').css('display', 'block');
    fnClearFields();
...
function fnSaveDoctorProfile() {
    ...
    if (IsStringNullorEmpty($('#txtDoctorRegnNo').val())) {
        toastr.warning("Please Enter Doctor Registration Number");
    ...
    if (IsStringNullorEmpty($('#cboPayoutType').val())) {
        toastr.warning("Please Select Payout Type");
        $('#cboPayoutType').focus();
        return;
    }
    if (IsStringNullorEmpty($('#cboSeniorityLevel').val())) {
        toastr.warning("Please Select Seniority Level");
