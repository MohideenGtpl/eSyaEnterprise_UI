...
using eSyaEnterprise_UI.Areas.Customer.Models;
...
namespace eSyaEnterprise_UI.Areas.Customer.Controllers
{
    [SessionTimeout]
    public class CustomerController : Controller
    {
        private readonly ICustomerAPIServices _eSyaCustomerAPIServices;
        private readonly ILogger<CustomerController> _logger;

        public CustomerController(ICustomerAPIServices eSyaCustomerAPIServices, ILogger<CustomerController> logger)
        {
        ...
        public IActionResult ECC_03_00()
        {
            return View();
        }
        ...
        /// <summary>
        /// Activate or De Activate Customer
        /// </summary>
        [HttpPost]
        public async Task<JsonResult> ActiveOrDeActiveCustomer(bool status, int customerId)
        {

            try
            {

                var parameter = "?status=" + status + "&customerId=" + customerId;
                var serviceResponse = await _eSyaCustomerAPIServices.HttpClientServices.GetAsync<DO_ReturnParameter>("CustomerMaster/ActiveOrDeActiveCustomer" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
            ...
            {
                _logger.LogError(ex, "UD:ActiveOrDeActiveCustomer:For customerId {0} ", customerId);
