    ...
    public class OpBillingController : Controller
    {
        ...
        int? clinicTypeId, int? patientTypeId, long? uhid, string patientname)
        {
            try
            {
                ...
                var serviceResponse = await _patientManagementAPIServices.HttpClientServices.GetAsync<List<DO_PatientRegisteredList>>("OpPatientVisitDetail/GetPatientRegisteredListbySearchCriteria" + parameter);
        ...
        public async Task<IActionResult> GetOpBillList(DateTime billFromDate, DateTime billTillDate, long? uhid, string patientname)
        {
            try
            {
                var parameter = "?businessKey=" + AppSessionVariables.GetSessionBusinessKey(HttpContext);
                parameter += "&billFromDate=" + billFromDate.ToString("dd-MMM-yy");
                parameter += "&billTillDate=" + billTillDate.ToString("dd-MMM-yy");
                ...
                var serviceResponse = await _patientManagementAPIServices.HttpClientServices.GetAsync<List<DO_OpBillDetail>>("OpBilling/GetOpBillListbySearchCriteria" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                {
