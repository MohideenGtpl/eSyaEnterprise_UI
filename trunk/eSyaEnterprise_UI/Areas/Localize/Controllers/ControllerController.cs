...
namespace eSyaEnterprise_UI.Areas.Localize.Controllers
{
    ...
    public class ControllerController : Controller
    {
        ...
        public async Task<IActionResult> ELE_03_00()
        {
            try
            {
                var serviceResponse = await _localizeAPIServices.HttpClientServices.GetAsync<List<string>>("Controller/GetAllControllers");
        ...
        public async Task<JsonResult> GetLanguageControllersbyResource(string Resource)
        {
            try
            {
                if (!string.IsNullOrEmpty(Resource))
                {
                    ...
                    var serviceResponse = await _localizeAPIServices.HttpClientServices.GetAsync<List<DO_LanguageController>>("Controller/GetLanguageControllersbyResource" + parameter);
        ...
        public async Task<JsonResult> InsertOrUpdateLanguageController(DO_LanguageController lobj)
        {
            ...
            try
            {
                ...
                var serviceResponse = await _localizeAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("Controller/InsertOrUpdateLanguageController", lobj);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
        ...
        public async Task<JsonResult> ActiveOrDeActiveLanguageController(bool status, int ResourceId)
        {
            ...
            try
            {
                ...
                var serviceResponse = await _localizeAPIServices.HttpClientServices.GetAsync<DO_ReturnParameter>("Controller/ActiveOrDeActiveLanguageController" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
