...
namespace eSyaEnterprise_UI.Areas.TokenSystem.Controllers
{
    ...
    public class DisplaySystemController : Controller
    {
        ...
        public async Task<IActionResult> ETM_11_00()
        {
            ...
            var serviceResponse = await _TokenSystemAPIServices.HttpClientServices.GetAsync<DO_DisplaySystemConfig>("DisplaySystem/GetDisplayConfigByIPAdddress" + parameter);
        ...
        public async Task<JsonResult> GetDisplayConfigByID(int DisplayId)
        {
            try
            {
                ...
                var serviceResponse = await _TokenSystemAPIServices.HttpClientServices.GetAsync<DO_DisplaySystemConfig>("DisplaySystem/GetDisplayConfigByID" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
