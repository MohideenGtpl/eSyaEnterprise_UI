 ...
 public class SKUController : Controller
    {
        ...
        public async Task<IActionResult> GetAccountingStoreForItem(int businesskey, int itemcode, string accountingType)
        {
            try
            {
                var store_serviceResponse = await _inventoryAPIServices.HttpClientServices.GetAsync<List<DO_ItemAccountingStore>>("ItemCodes/GetStoreClassStorebyBusinessKey?businesskey=" + businesskey);
                var parameter = "?Businesskey=" + businesskey + "&Itemcode=" + itemcode + "&actype=" + accountingType;

                var serviceResponse = await _inventoryAPIServices.HttpClientServices.GetAsync<List<DO_ItemAccountingStore>>("ItemCodes/GetCustodianStoreListbyAccountType" + parameter);

                List<DO_ItemAccountingStore> _masterdata = store_serviceResponse.Data;
                List<DO_ItemAccountingStore> _linkeddata = serviceResponse.Data;

                if (serviceResponse.Status && store_serviceResponse.Status)
                {
                    if (_masterdata.Count > 0)
                    {
                       var _filtermasterdata = _masterdata.Where(x => x.AccountingType.Trim() == accountingType.Trim());
                        foreach (var obj in _filtermasterdata)
                        {
                           
                                if (_linkeddata.Count > 0)
                                {
                                    var _isExists = _linkeddata
                                          .Where(c => c.BusinessKey == businesskey && c.ItemCode == itemcode  && c.AccountingType.Trim()==accountingType.Trim()).FirstOrDefault();
                        ...
                        return Json(_filtermasterdata);
                    }
                    else
                    {
                        return Json(_masterdata);

                    }
                }
                else
                {
                    _logger.LogError("UD:GetAccountingStoreForItem:For businesskey {0} with itemcode entered {1}", businesskey, itemcode);
                    return Json(new { Status = false, StatusCode = "500" });
                }
            }
            catch (Exception ex)
        ...
        public async Task<IActionResult> GetCustodianStoreForItem(int businesskey, int itemcode,  string accountingType)
        {
            try
            {
                var store_serviceResponse = await _inventoryAPIServices.HttpClientServices.GetAsync<List<DO_ItemAccountingStore>>("ItemCodes/GetStoreClassStorebyBusinessKey?businesskey=" + businesskey);
        ...
        public async Task<IActionResult> GetConsumtptionStoreForItem(int businesskey, int itemcode,  string accountingType)
        {
            try
            {
                var store_serviceResponse = await _inventoryAPIServices.HttpClientServices.GetAsync<List<DO_ItemAccountingStore>>("ItemCodes/GetStoreClassStorebyBusinessKey?businesskey=" + businesskey);
        ...
        public async Task<IActionResult> GetSalesStoreForItem(int businesskey, int itemcode, string accountingType)
        {
            try
            {
                var store_serviceResponse = await _inventoryAPIServices.HttpClientServices.GetAsync<List<DO_ItemAccountingStore>>("ItemCodes/GetStoreClassStorebyBusinessKey?businesskey=" + businesskey);
