    ...
    public class EmployeeExpatController : Controller
    {
        ...
        public async Task<IActionResult> V_4500_00()
        {
            try
            {
                ...
                l_ac.Add(CodeTypeValues.EmployeeLocation);
                ...
                var Bk_response2 = await _eSyaPayrollExpatAPIServices.HttpClientServices.GetAsync<List<DO_CurrencyMaster>>("CommonData/GetActiveCurrencyList");

                if (response.Status && Bk_response.Status && Bk_response2.Status)
                {
                    if (response.Data != null && Bk_response.Data != null && Bk_response2.Data != null)
                    {
                        ...
                        ViewBag.CurrencyList = Bk_response2.Data.Select(b => new SelectListItem
                        {
                            Value = b.CurrencyCode.ToString(),
                            Text = b.CurrencyName.ToString(),
                        }).ToList();
                        ...
                        ViewBag.EmployeeLocationList = app_codes.Where(w => w.CodeType == CodeTypeValues.EmployeeLocation).Select(b => new SelectListItem
                        {
                            Value = b.ApplicationCode.ToString(),
                            Text = b.CodeDesc,
                        }).ToList();
        ...
        public async Task<JsonResult> InsertIntoSalaryInfo(DO_SalaryInfo obj)
        {
            try
            {
                ...
                else if (string.IsNullOrEmpty(obj.SalaryCurrency.ToString()))
                {
        ...
        /// </summary>
        [HttpPost]
        public async Task<JsonResult> GetFixedDeductionInfo(int BusinessKey,int EmployeeNumber)
        {
            try
            {
                var parameter = "?BusinessKey=" + BusinessKey + "&EmployeeNumber=" + EmployeeNumber;
                var serviceResponse = await _eSyaPayrollExpatAPIServices.HttpClientServices.GetAsync<List<DO_FixedDeductionInfo>>("EmployeeMaster/GetFixedDeductionInfo" + parameter);
                ...
                if (serviceResponse.Status)
                {
                    ...
                    {
                        _logger.LogError(new Exception(serviceResponse.Message), "UD:GetFixedDeductionInfo:For BusinessKey {0} With EmployeeNumber entered {1}", BusinessKey, EmployeeNumber);
                        return Json(new { Status = false, StatusCode = "500" });
                    }
                }
                else
            ...
            {
                _logger.LogError(ex, "UD:GetFixedDeductionInfo:For BusinessKey {0} With EmployeeNumber entered {1}", BusinessKey, EmployeeNumber);
                throw ex;
            }
        }
        ...
        /// </summary>
        [HttpPost]
        public async Task<JsonResult> InsertOrUpdateFixedDeductionInfo(DO_FixedDeductionInfo obj)
        {
            try
            {
                obj.UserID = AppSessionVariables.GetSessionUserID(HttpContext);
                obj.TerminalID = AppSessionVariables.GetIPAddress(HttpContext);
                obj.FormId = AppSessionVariables.GetSessionFormInternalID(HttpContext);
                var serviceResponse = await _eSyaPayrollExpatAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("EmployeeMaster/InsertOrUpdateFixedDeductionInfo", obj);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
            ...
            {
                _logger.LogError(ex, "UD:InsertOrUpdateFixedDeductionInfo:params:" + JsonConvert.SerializeObject(obj));
