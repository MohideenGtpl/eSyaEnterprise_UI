...
namespace eSyaEnterprise_UI.Areas.DoctorScheduler.Controllers
{
    ...
    public class AppointmentController : Controller
    {
        ...
        public async Task<IActionResult> EDA_01_00()
        {
            try
            {
                var serviceResponse = await _doctorAppointmentAPIServices.HttpClientServices.GetAsync<List<DO_BusinessLocation>>("Master/GetBusinessKey");
                if (serviceResponse.Status)
                {
                    ViewBag.BusinessLocation = serviceResponse.Data;
        ...
        [Area("DoctorScheduler")]
        [ServiceFilter(typeof(ViewBagActionFilter))]
        public async Task<IActionResult> EDA_02_00()
        {
            //try
            //{
            //    var serviceResponse = await _doctorAppointmentAPIServices.HttpClientServices.GetAsync<List<DO_BusinessLocation>>("Master/GetBusinessKey");
            ...
            //        {
            //            Text = a.BusinessLocation,
            //            Value = a.BusinessKey.ToString(),
            //            Selected = a.BusinessKey == AppSessionVariables.GetSessionBusinessKey(HttpContext)
            //        });
        ...
        public async Task<JsonResult> GetEpisodeType()
        {
            try
            {
                var serviceResponse = await _doctorAppointmentAPIServices.HttpClientServices.GetAsync<List<DO_EpisodeType>>("OpClinicDetails/GetEpisodeType");
                if (serviceResponse.Status)
        ...
        public async Task<JsonResult> GetClinicConsultationTypeByBKey(int businessKey)
        {
            try
            {
                ...
                var serviceResponse = await _doctorAppointmentAPIServices.HttpClientServices.GetAsync<List<DO_ClinicType>>("OpClinicDetails/GetClinicTypes?businessKey=" + businessKey);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                {
        ...
        public async Task<JsonResult> GetSpecialtyList(int businessKey)
        {
            try
            {
                if(businessKey==0)
                ...
                var serviceResponse = await _doctorAppointmentAPIServices.HttpClientServices.GetAsync<List<DO_Specialty>>("OpClinicDetails/GetSpecialty?businessKey=" + businessKey);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                {
        ...
        public async Task<JsonResult> GetDoctorScheduleListByClinicTypeSpecialtyDate(int clinicType, int consultationType,
            int specialtyId, DateTime scheduleDate,int businessKey)
        {
            try
            {
                if (businessKey == 0)
        ...
        public async Task<JsonResult> GetDoctorListByClinicType(int clinicType, int consultationType,
          int specialtyId,int businessKey)
        {
            try
            {
                if (businessKey == 0)
        ...
        int specialtyId, int doctorId,int businessKey)
        {
            try
            {
                if (businessKey == 0)
        ...
        public async Task<JsonResult> GetDoctorAvailableTimeSlotGivenDate(int clinicType, int consultationType, int specialtyId, int doctorId, DateTime appointmentDate,int businessKey)
        {
            try
            {
                if (businessKey == 0)
                ...
                parameter += "&specialtyId=" + specialtyId.ToString();
                parameter += "&doctorId=" + doctorId.ToString();
                parameter += "&fromDate=" + appointmentDate.ToString("dd-MMM-yyyy");
                parameter += "&toDate=" + appointmentDate.ToString("dd-MMM-yyyy");
                ...
                foreach (var t in serviceResponse_schedule.Data)
                {
                    ...
                    {
                        TimeSpan duration = TimeSpan.FromMinutes(t.TimeSlotInMintues);
        ...
        public async Task<JsonResult> GetDoctorScheduleHours(int clinicType, int consultationType, int specialtyId, string doctorId, DateTime scheduleDate, int businessKey)
        {
            try
            {
                ...
                TimeSpan timeSlotInMintues = new TimeSpan();
                ...
                foreach (string d in doctorId.Split(','))
                {
                    ...
                    if (d != null)
                    {
                        ...
                        if (serviceResponse.Status)
                        {
                            ...
                            foreach (var s in serviceResponse.Data)
                            {
                                ...
                                timeSlotInMintues = new TimeSpan(0, s.TimeSlotInMintues, 0);
                            }
                    ...
                    if (l_rs.Count() > 0)
                    {
                        ...
                        return Json(new { Duration = timeSlotInMintues.ToString(), minDate, maxDate, businessHours = l_rs });
                    }
                    else
                        // return Json(new { Duration = "00:05", minDate = "00:00:00", maxDate = "00:00:00", businessHours = l_rs });
                        return Json(new { Duration = timeSlotInMintues.ToString(), minDate = "00:00:00", maxDate = "00:00:00", businessHours = l_rs });
                }
        ...
        public async Task<JsonResult> InsertIntoPatientAppointmentDetail(DO_PatientAppointmentDetail obj)
        {
            try
            {
                ...
                {
                    if (serviceResponse.Data.Status)
                        return Json(new { Status = true, QTokenKey = serviceResponse.Data.strKey });
                    else
        ...
        [HttpGet]
        public async Task<JsonResult> GetDoctorUnScheduleWorkOrder(int specialtyId, int doctorId,int businessKey)
        {
            try
            {
                if (businessKey == 0)
        ...
        public async Task<JsonResult> GetPatientAppointmentByDoctorDate(int specialtyId,
            string doctorId, DateTime start, DateTime end,int businessKey)
        {
            try
            {
        ...
        public async Task<JsonResult> GetApplicationRuleByProcesssID()
        {
            try
            {
                var param = "processId=" + 4;
                param += "&ruleId=" + 1;
                var serviceResponse = await _doctorAppointmentAPIServices.HttpClientServices.GetAsync<DO_ApplicationRulesStatus>("Appointment/GetApplicationRuleByProcesssID?" + param);
                if (serviceResponse.Status)
                {
                    return Json(serviceResponse.Data);
                }
                ...
                {
                    _logger.LogError(new Exception(serviceResponse.Message), string.Format("UD:GetApplicationRuleByProcesssID:params:processID:{0}:ruleId:{1}"));
                    return Json(new DO_ResponseParameter() { Status = false, Message = serviceResponse.Message });
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, string.Format("UD:GetApplicationRuleByProcesssID:params:processID:{0}:ruleId:{1}" ));
                return Json(new { Status = false, Warning = false, Message = ex.InnerException == null ? ex.Message.ToString() : ex.InnerException.Message });
            }
        }
