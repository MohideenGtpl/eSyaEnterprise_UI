...
namespace eSyaEnterprise_UI.Areas.eSyaOutPatient.Controllers
{
    ...
    public class AppointmentSchedularController : Controller
    {
        ...
        public async Task<JsonResult> GetReferredBy()
        {
            try
            {
                var serviceResponse = await _eSyaOutPatientAPIServices.HttpClientServices.GetAsync<List<DO_AppCodes>>("CommonMaster/GetApplicationCode?codeType="+ eSyaEnterprise_UI.Areas.eSyaOutPatient.Data.CodeTypeValues.ReferredBy.ToString());
        ...
        public async Task<JsonResult> GetClinicConsultationTypeByBKey()
        {
            try
            {
                var serviceResponse = await _eSyaOutPatientAPIServices.HttpClientServices.GetAsync<List<DO_ClinicConsultationType>>("DoctorClinic/GetClinicConsultationTypeByBKey?businessKey=" + AppSessionVariables.GetSessionBusinessKey(HttpContext));
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                {
        ...
        public async Task<JsonResult> GetSpecialtyListByBKey()
        {
            try
            {
                var serviceResponse = await _eSyaOutPatientAPIServices.HttpClientServices.GetAsync<List<DO_Specialty>>("DoctorClinic/GetSpecialtyListByBKey?businessKey=" + AppSessionVariables.GetSessionBusinessKey(HttpContext));
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                {
        ...
        public async Task<JsonResult> SaveRegisterPatientAppointment(DO_PatientAppointmentDetail obj)
        {
            try
            {
                ...
                var serviceResponse = await _eSyaOutPatientAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("OPRegistration/InsertInToPatientOPRegistration", obj);
                if (serviceResponse.Status)
                {
                    if (serviceResponse.Data.Status)
                        return Json(new { Status = true });
        ...
        public async Task<JsonResult> GetPatientSearch(string searchText)
        {
            try
            {
                ...
                var serviceResponse = await _eSyaOutPatientAPIServices.HttpClientServices.GetAsync<List<DO_PatientProfile>>("OPRegistration/GetPatientSearch" + parameter);
                if (serviceResponse.Status)
                {
