        ...
        colModel: [
...

            {
                ...
                formatter: function (cellValue, options, rowdata, action) {
                    ...
                    return '<button class="btn-xs ui-button ui-widget ui-corner-all btn-jqgrid" title="Edit" id="jqgEdit" onclick="return fnEditVoucher(event,\'edit\')"><i class="fas fa-pen"></i> ' + localization.Edit + '</button>' +
                        '<button class="btn-xs ui-button ui-widget ui-corner-all btn-jqgrid" title="View" id="jqgView" onclick="return fnEditVoucher(event,\'view\')"><i class="far fa-eye"></i> ' + localization.View + '</button>' +
                        '<button class="btn-xs ui-button ui-widget ui-corner-all btn-jqgrid" title = "Delete" id="jqgDelete" onclick = "return fnEditVoucher(event,\'delete\')" > <i class="fas fa-trash"></i>' + localization.Delete + '</button>'
                }
            },
        ],
...
function fnEditVoucher(e, actiontype) {
    ...
    if (actiontype.trim() == "edit") {
        ...
        $("#btnSaveVoucher").html(localization.Update);
        $("#btnSaveVoucher").attr('disabled', false);
        $("#btnSaveVoucher").show();
        $("#btnDeactivateVoucher").hide();
        $("#chkActiveStatus").prop('disabled', true);
    ...
    if (actiontype.trim() == "delete") {
        ...
        $("select").next().attr('disabled', true);
        $("#btnSaveVoucher").hide();
        $("#btnDeactivateVoucher").html(localization.Delete);
...
function fnSaveVoucher() {
    if (IsStringNullorEmpty($("#txtVoucherId").val())) {
        toastr.warning("Please Enter Voucher Id");
        return;
    }
    if (IsStringNullorEmpty($("#txtTransactionType").val())) {
        toastr.warning("Please Enter Transaction Type");
        return;
    }
    if (IsStringNullorEmpty($("#txtVoucherDes").val())) {
        toastr.warning("Please Enter Voucher Description");
        return;
    }
    obj_voucher = {
