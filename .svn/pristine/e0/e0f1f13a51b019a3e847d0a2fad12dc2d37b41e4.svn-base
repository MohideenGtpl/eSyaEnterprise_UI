...
namespace eSyaEnterprise_UI.Areas.Configure.Controllers
{
    ...
    public class FormNamesController : Controller
    {
        ...
        [Area("Configure")]
        //[ServiceFilter(typeof(ViewBagActionFilter))]
        public IActionResult ECA_01_01()
        {
        ...
        public async Task<JsonResult> GetAllAreaController()
        {
            ...
            try
            {
                var serviceResponse = await _configAPIServices.HttpClientServices.GetAsync<List<DO_AreaController>>("Forms/GetAllAreaController");
                if (serviceResponse.Status)
                {
                    return Json(serviceResponse.Data);
                }
        ...
        public async Task<JsonResult> InsertOrUpdateAreaController(bool isInsert, DO_AreaController obj)
        {
            ...
            try
            {
                if (isInsert)
                {
                    var serviceResponse = await _configAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("Forms/InsertIntoAreaController", obj);
                    if (serviceResponse.Status)
                        return Json(serviceResponse.Data);
                    else
                        return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
                ...
                {
                    var serviceResponse = await _configAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("Forms/UpdateAreaController", obj);
                    if (serviceResponse.Status)
                        return Json(serviceResponse.Data);
                    else
                        return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
        ...
        public async Task<JsonResult> ActiveOrDeActiveAreaController(bool status, int Id)
        {
            ...
            try
            {
                ...
                var serviceResponse = await _configAPIServices.HttpClientServices.GetAsync<DO_ReturnParameter>("Forms/ActiveOrDeActiveAreaController" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
