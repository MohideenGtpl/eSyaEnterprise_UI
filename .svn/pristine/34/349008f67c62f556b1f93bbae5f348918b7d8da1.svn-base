...
namespace eSyaEnterprise_UI.Areas.eSyaClinicalServices.Controllers
{
    ...
    public class DoctorController : Controller
    {
        ...
        public DoctorController(IeSyaClinicalAPIServices eSyaClinicalAPIServices, ILogger<DoctorController> logger, IHostingEnvironment hostingEnvironment)
        {
        ...
        public async Task<JsonResult> ActiveOrDeActiveDoctor(bool status, int doctorId)
        {
            ...
            try
            {
                ...
                var serviceResponse = await _eSyaClinicalAPIServices.HttpClientServices.GetAsync<DO_ReturnParameter>("DoctorMaster/ActiveOrDeActiveDoctor" + parameter);
        ...
        public async Task<JsonResult> ActiveOrDeActiveDoctorScheduler(bool status,int businesskey,int consultationId,int clinicId,int specialtyId, int doctorId,int serialNo)
        {
            ...
            try
            {
                ...
                var serviceResponse = await _eSyaClinicalAPIServices.HttpClientServices.GetAsync<DO_ReturnParameter>("DoctorSchedule/ActiveOrDeActiveDoctorScheduler" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
        ...
        [Area("eSyaClinicalServices")]
        [ServiceFilter(typeof(ViewBagActionFilter))]
        public IActionResult V_1554_00()
        {
        ...
        public async Task<JsonResult> GetDoctordetailsbydoctorId(int doctorId)
        {
            try
            {
                var serviceResponse = await _eSyaClinicalAPIServices.HttpClientServices.GetAsync<Do_DoctorDetails>("DoctorMaster/GetDoctordetailsbydoctorId?doctorId=" + doctorId);
                if (serviceResponse.Status)
                {
        ...
        public async Task<JsonResult> InsertOrUpdateIntoDoctordetails(Do_DoctorDetails obj)
        {
            try
            {
                ...
                var serviceResponse = await _eSyaClinicalAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("DoctorMaster/InsertOrUpdateIntoDoctordetails", obj);
                if (serviceResponse.Status)
                {
                    return Json(serviceResponse.Data);
                }
