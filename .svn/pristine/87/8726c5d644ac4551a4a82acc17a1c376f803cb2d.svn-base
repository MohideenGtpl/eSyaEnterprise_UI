...
namespace eSyaEnterprise_UI.Areas.eSyaFixedAssets.Controllers
{
    ...
    public class FixedAssetsController : Controller
    {
        ...
        [Area("FixedAssets")]
        [ServiceFilter(typeof(ViewBagActionFilter))]
        public async Task<IActionResult> V_2002_00()
        {

            try
            {
                var serviceResponse = await _FixedAssetsAPIServices.HttpClientServices.GetAsync<List<DO_ISDCodes>>("CommonData/GetISDCodes");
                var response = await _FixedAssetsAPIServices.HttpClientServices.GetAsync<List<DO_ApplicationCodes>>("CommonData/GetApplicationCodesByCodeType?codeType=" + CodeTypeValues.AssetGroup);
                if (response.Status && serviceResponse.Status)
                {
                    if (response.Data != null)
                    {
                        ViewBag.AssetGroup = response.Data.Select(b => new SelectListItem
                        {
                            Value = b.ApplicationCode.ToString(),
                            Text = b.CodeDesc,
                        }).ToList();
                    ...
                    {
                        ViewBag.ISDCodes = serviceResponse.Data.Select(b => new SelectListItem
                        {
                ...
                {
                    _logger.LogError(new Exception(response.Message), "UD:GetApplicationCodesByCodeType");
                }
                return View();
            }
            catch (Exception ex)
        ...
        /// </summary>
        [HttpPost]
        public async Task<JsonResult> GetAssetGroupforGrid(int AssetId,int Isdcode)
        {

            try
            {
                var response = await _FixedAssetsAPIServices.HttpClientServices.GetAsync<List<DO_AssetGroup>>("AssetGroup/GetAssetGroupforGrid?AssetId="+ AssetId+ "&Isdcode="+ Isdcode);
                if (response.Status)
                {
        ...
        /// </summary>
        [HttpPost]
        public async Task<JsonResult> GetAssetDepreciationMethodsbyAssGroupId(int assetGroupId,int assetSubGroupId,int Isdcode)
        {
            try
            {
                var response = await _FixedAssetsAPIServices.HttpClientServices.GetAsync<List<DO_AssetDepreciationMethod>>("AssetGroup/GetAssetDepreciationMethodsbyAssGroupId?assetGroupId=" + assetGroupId + "&assetSubGroupId=" + assetSubGroupId + "&Isdcode=" + Isdcode);
                if (response.Status)
                    return Json(response.Data);
                else
                {
